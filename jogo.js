...
var select = function(value){
	...
	if (movement.length == 2){
		...
		//onsole.log("movimento possível " + movement);
		}
		...
		//Aqui é a vez do monstro atacar (será necessário tratar a seleção do monstro... )
...
/************************************MONSTER*****************************/
	...
	var monsterPosition = 12;
		...
		var monsterMoving = function(coord){
			...
			console.log("PIF 236"+ parseInt(coord[0]));
			
			console.log("nnn"+ coord[1]);
			positionIsFree[parseInt(coord[1])] = false;   /************************/
			console.log("PIF 242"+ parseInt(coord[1]));
			...
			console.log("movimento do monstro validado na linha 236");
				...
				monsterSearch2 = canMove[monsterPosition];
		...
		console.log("monsterMovement 271: "+monsterMovement);
		...
		if (attackSelect.length > 0){
			...
			console.log("SMS "+ simpleMoveSelect[0]);
			...
			let dice = 1 + (Math.floor(Math.random() * 6)) ;
      			...
      			if (tam >= 2 && dice >=5){
				for (j=tam-1; j>0; j--){
					console.log("simpleMoveSelect["+j+"] = "+ simpleMoveSelect[j]);
					console.log(position[simpleMoveSelect[j]][0])
					console.log("menor posição:"+ simpleMoveSelect[0]);
					console.log(position[simpleMoveSelect[0]][0])
			...
			//monsterMovement.push(simpleMoveSelect[0]);
			console.log("monsterMovement  SMS 289: "+monsterMovement);
			attack = false;
			monsterMoving(monsterMovement);
		}else if(monsterPosition == 26){alert("Congratulations, you win!!!")};
